...
public static class ExtMathf
{
	...
	public static float CustomLerpFloat(float frm, float to, float speed, float deltaTime, float snapTolerance)
	{
		float res = Mathf.Lerp(frm, to, speed * deltaTime);

		if (Mathf.Abs(to - frm) < snapTolerance)
		{
			res = to;
		}
