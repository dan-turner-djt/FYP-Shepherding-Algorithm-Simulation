...
public static class ExtCollider
{
	...
	#region Sphere
	public static List<ContactInfo> ClosestPointsOnSurface(Collider collider, Vector3 centre, float radius, List<ContactInfo> resultsBuffer, bool multipleContacts = true, bool forWalls = false, bool forGroundCast = false)
	{
		resultsBuffer.Clear();
		...
		else if (collider is BoxCollider) resultsBuffer.Add(ClosestPointOnSurface((BoxCollider)collider, centre, forWalls));
	...
	public static ContactInfo ClosestPointOnSurface(BoxCollider collider, Vector3 centre, bool forWalls = false)
	{
		Vector3 localCentre = ToLocal(collider, collider.center, centre);
		...
		if (forWalls)
        {
			yDistance = Mathf.Infinity; //this is so that the yNormal will never be chosen if we are wanting a wall normal
        }
		...
		if (ExtMathf.Approximately(xDistance, yDistance, .0001f) || ExtMathf.Approximately(xDistance, zDistance, .0001f) || ExtMathf.Approximately(yDistance, zDistance, .0001f))
		{
			...
			if (forWalls)
			{
				yDot = 0; //this is so that the yNormal will never be chosen if we are wanting a wall normal
			}
			...
			else if (yDot >= xDot && yDot >= zDot && !forWalls)
			{
				closestNormal = yAxis;
			}
			else
