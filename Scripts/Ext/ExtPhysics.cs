...
public static class ExtPhysics
{
	...
	public static bool CheckSphere(Vector3 centre, float radius, IList<Component> componentsLayers, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, Inclusion layerContext = Inclusion.Ignore)
	//public static bool CheckSphere(Vector3 centre, float radius, IList<Component> componentsLayers, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		int mask = GetMaskAndStoreOriginalLayerAndReassign(layerMask, layerContext, componentsLayers);
		bool hasHit = CheckSphere(centre, radius, layerMask, queryTriggerInteraction);
	...
	public static bool CheckSphere(Vector3 centre, float radius, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		return Physics.CheckSphere(centre, radius, layerMask & ExtLayerMask.ignoreRaycastMask, queryTriggerInteraction);
	}
	#endregion
	...
	public static bool CheckCapsule(Vector3 origin, Vector3 direction, float height, float radius, IList<Component> componentsLayers, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		CapsuleShape points = new CapsuleShape(origin, direction, height, radius);
	...
	//public static bool CheckCapsule(Vector3 segment0, Vector3 segment1, float radius, IList<Component> componentsLayers, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		//int mask = GetMaskAndStoreOriginalLayerAndReassign(layerMask, layerContext, componentsLayers);
		bool hasHit = CheckCapsule(segment0, segment1, radius, layerMask, queryTriggerInteraction);
		//RestoreToOriginalLayer(componentsLayers);
		return hasHit;
	}
	...
	public static bool CheckCapsule(Vector3 segment0, Vector3 segment1, float radius, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		...
		//return Physics.CheckCapsule(segment0, segment1, radius, layerMask, queryTriggerInteraction);
	}
	#endregion
	...
	//public static IList<Collider> OverlapSphere(Vector3 centre, float radius, IList<Component> componentsLayers, IList<Collider> resultBuffer, int layerMask, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	public static IList<Collider> OverlapSphere(Vector3 centre, float radius, IList<Component> componentsLayers, IList<Collider> resultBuffer, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, Inclusion layerContext = Inclusion.Ignore)
	{
		int mask = GetMaskAndStoreOriginalLayerAndReassign(layerMask, layerContext, componentsLayers);
		OverlapSphere(centre, radius, resultBuffer, layerMask, queryTriggerInteraction);
		RestoreToOriginalLayer(componentsLayers);
		return resultBuffer;
	}
	...
	public static IList<Collider> OverlapSphere(Vector3 centre, float radius, IList<Collider> resultBuffer, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		resultBuffer.Clear();
		int found = Physics.OverlapSphereNonAlloc(centre, radius, nonAllocateOverlapSphereResults, layerMask & ExtLayerMask.ignoreRaycastMask, queryTriggerInteraction);
		//int found = Physics.OverlapSphereNonAlloc(centre, radius, nonAllocateOverlapSphereResults, layerMask, queryTriggerInteraction);
	...
	{
		int mask = GetMaskAndStoreOriginalLayerAndReassign(layerMask, layerContext, componentsLayers);
		RaycastHit hitInfo = SphereCast(origin, radius, direction, maxDistance, layerMask, queryTriggerInteraction);
		RestoreToOriginalLayer(componentsLayers);
		return hitInfo;
	}
	...
	/*#region RayCast
	public static RaycastHit RayCast(Vector3 origin, Vector3 direction, Component componentLayer, float maxDistance = Mathf.Infinity, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, Inclusion layerContext = Inclusion.Ignore)
	{
		SetTempComponentLayer(componentLayer);
		return SphereCast(origin, direction, tempComponentLayer, maxDistance, layerMask, queryTriggerInteraction, layerContext);
	}
	public static RaycastHit RayCast(Vector3 origin, Vector3 direction, IList<Component> componentsLayers, float maxDistance = Mathf.Infinity, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, Inclusion layerContext = Inclusion.Ignore)
	{
		int mask = GetMaskAndStoreOriginalLayerAndReassign(layerMask, layerContext, componentsLayers);
		RaycastHit hitInfo = SphereCast(origin, direction, maxDistance, mask, queryTriggerInteraction);
		RestoreToOriginalLayer(componentsLayers);
		return hitInfo;
	}
	public static RaycastHit RayCast(Vector3 origin, Vector3 direction, float maxDistance = Mathf.Infinity, int layerMask = Physics.AllLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
		RaycastHit hit;
		Physics.SphereCast(origin, direction, out hit, maxDistance, layerMask & ExtLayerMask.ignoreRaycastMask, queryTriggerInteraction);
		return hit;
	}
	#endregion*/
