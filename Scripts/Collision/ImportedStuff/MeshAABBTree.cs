...
public class MeshAABBTree : MonoBehaviour
{
	...
	void Awake()
	{
		...
		Debug.Log("done mesh tree");
	}


	void Update()
	{
		//Debug.Log("mesh tree update");
	...
	public List<ContactInfo> ClosestPointsOnSurface(Vector3 point, float radius, List<ContactInfo> resultsBuffer, bool forGroundCast = false)
	{
		resultsBuffer.Clear();
		...
		CleanUp(closestPointsBufferMultiSphere, resultsBuffer, forGroundCast);
	...
	public List<ContactInfo> CollectNormalsOnCollider(Vector3 point, float radius, List<ContactInfo> resultsBuffer)
	{
		resultsBuffer.Clear();
		trianglesBufferMultiSphere.Clear();
		closestPointsBufferMultiSphere.Clear();
		...
		while (enumerator.MoveNext())
		{
			...
			if (distance <= radiusSqrd)
			{
				//if (Geometry.CheckPointIsOnTrianglePerimeter (localPoint, p1, p2, p3)) 
				//{
				closestPointsBufferMultiSphere.Add(new ClosestTrianglePoint(nearest, distance, enumerator.Current, localPoint, this));
				//}
			}
		...
		//Debug.Log ("closest triangle points:" + closestPointsBufferMultiSphere.Count);
		CleanUpForEdges(closestPointsBufferMultiSphere, resultsBuffer);
		//Debug.Log (resultsBuffer.Count);
	...
	void CleanUp(List<ClosestTrianglePoint> closestPoints, List<ContactInfo> resultsBuffer, bool forGroundCast = false)
	{
		if (closestPoints.Count > 0)
		{
			...
			for (int i = 0; i < closestPoints.Count; i++)
			{
				...
				if (!forGroundCast || true)
				{
					...
					//(although inefficient) to ignore cleaning completely
					//note: ignoring cleaning for standard penetration checks seem to result in glitchy behaviour for some reason, so we continue to clean for normal collision
	...
	void CleanUpForEdges(List<ClosestTrianglePoint> closestPoints, List<ContactInfo> resultsBuffer)
	{
		resultsBuffer.Clear();
		...
		if (closestPoints.Count > 0)
		{
			...
			for (int i = 0; i < closestPoints.Count; i++)
			{
				...
				resultsBuffer.Add(new ContactInfo(closestPoint.position, transform.TransformDirection(planeNormal)));
			}
		}
	}
