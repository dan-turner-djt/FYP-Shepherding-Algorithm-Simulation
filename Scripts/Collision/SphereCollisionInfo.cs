...
public struct SphereCollisionInfo
{
	...
	public SphereCollisionInfo(bool hasCollided, Collider collider, Vector3 detectionOrigin, Vector3 realOrigin, float sphereRadius, Vector3 closestPointOnSurface, Vector3 normal, Vector3 transformUp)
	{
	...
	public static float GetCollisionMagnitudeInDirection(Vector3 collisionVelocity, Vector3 normal, float radius, float heightAdjustedRadialDistance)
	{
		float mag = -ExtVector3.MagnitudeInDirection(collisionVelocity, normal, false);
		mag = Mathf.Clamp(mag, -radius, radius);
		return heightAdjustedRadialDistance + mag;
	}
	...
	public static float GetHeightAdjustedRaidalDistance(Vector3 origin, Vector3 closestPoint, Vector3 up, float radius)
	{
		float height = Mathf.Abs(ExtVector3.MagnitudeInDirection(origin - closestPoint, up, false));
		height = Mathf.Clamp(height, 0, radius);
		float heightAdjustedRadialDistance = Mathf.Sqrt(radius * radius - height * height);
		heightAdjustedRadialDistance = Mathf.Clamp(heightAdjustedRadialDistance, 0, radius);
		return heightAdjustedRadialDistance;
	}
