...
public class MM_SheepLoadOld : GenericMenu
{
	...
	{
		files = uiStackManager.sc.systemController.GetValidFilesInDirectory(1);
	...
	void UpdateOtherFields()
	{
		...
		uiStackManager.sc.systemController.SetSimulationSettingsFromFileName(currentName, 1);
	...
	public void DeleteButtonPressed()
	{
		if (files.Count > 0)
		{
			...
			string used = uiStackManager.sc.systemController.CheckIfSettingsUsedInSimulation(fileName, 1);


			if (!used.Equals(""))
            {
				string msg = "Cannot delete because these settings are used in " + used;
	...
	public void DeleteButtonAction(bool confirmed)
	{
		if (confirmed)
		{
			...
			uiStackManager.sc.systemController.DeleteFile(fileName, 1);
